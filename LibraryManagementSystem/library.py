...
class temp():
    ...
    to_be_returned=datetime.date.today()
    issued_by=''

    def __init__(self,books):
        self.name=books.name
        self.author=books.author
        self.quantity=books.quantity
        self.section=books.section
        self.serialno=books.serialno
        self.issued_on=books.issued_on
        self.to_be_returned=books.to_be_returned
        self.issued_by=books.issued_by
...
def get_books_user(user):
    ...
    for sno in user.books:
        lsno=[sno]
        book = Book.query.filter(Book.serialno.contains_(lsno)).first()
...
def search_P(Query):
    if  'author' in Query:
        ...
        firstset=set()
        secondset=set()
        for mm in Book.query.filter(Book.name.regex(Query)).all():
            t = temp(mm)
            firstset.add(t)
        for mm in Book.query.filter(Book.author.regex(Query)).all():
            t = temp(mm)
            secondset.add(t)
        books=list(firstset|secondset)
        return search_object(books,list(),None)
